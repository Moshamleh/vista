...
var OpportunityRepository = class {
  ...
  /** Lists opportunities. */
  async list(limit) {
    const result2 = await this.db.prepare("select * from opportunities order by updated_at desc limit ?").bind(limit).all();
    return result2.results.map(mapOpportunity);
  }
  /** Finds one opportunity. */
  async findById(id) {
    const row = await this.db.prepare("select * from opportunities where id = ?").bind(id).first();
...
async function routeRequest(request, env) {
  ...
  try {
    ...
    const headers = { "x-trace-id": traceId, "x-api-version": container.config.apiVersion };
    ...
    return failureResponse(toAppError(error), context.requestId);
  }
}
...
// .wrangler/tmp/bundle-vFUJYs/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vFUJYs/middleware-loader.entry.ts
